...
class EvalSample:
    ...
    persistence_pbias: float
    persistence_nse: float
    model_mae: float
    model_rmse: float
    model_pbias: float
    model_nse: float
...
def per_lead_pbias(
    ...
    out = []
    for lead_idx in range(pred.shape[0]):
        p = np.ma.array(pred[lead_idx])
        t = np.ma.array(tgt[lead_idx])
        denom = float(np.ma.sum(t))
        if abs(denom) < 1e-12:
            out.append(np.nan)
        else:
            out.append(float(100.0 * np.ma.sum(p - t) / denom))
...
def per_lead_nse(
    ...
    for lead_idx in range(pred.shape[0]):
        p = np.ma.array(pred[lead_idx])
        t = np.ma.array(tgt[lead_idx])
        t_mean = float(np.ma.mean(t))
        num = float(np.ma.sum((p - t) ** 2))
        den = float(np.ma.sum((t - t_mean) ** 2))
        if den < 1e-12:
            out.append(np.nan)
        else:
            out.append(float(1.0 - num / den))
    return np.asarray(out, dtype=np.float64)
...
def load_soilcast(
    *,
    ...
    transformer = infer_transformer_from_ckpt(soilcast_ckpt)
...
def save_panel(
    *,
    ...
    plot_era5land_panel(
        ...
        row_titles=["Ground truth", "Persistence", "SoilCast"],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch evaluation for ERA5-Land SoilCast rollouts.")
    parser.add_argument("--soilcast-ckpt", "--gpt-ckpt", dest="soilcast_ckpt", type=Path, required=True)
...
def main() -> None:
    ...
    model = load_soilcast(
        soilcast_ckpt=args.soilcast_ckpt,
    ...
    print(f"SoilCast checkpoint: {args.soilcast_ckpt}")
    ...
    for order, sample_idx in enumerate(indices):
        ...
        persist_pbias = per_lead_pbias(persist_phys, target_phys)
        persist_nse = per_lead_nse(persist_phys, target_phys)
        model_mae, model_rmse = per_lead_mae_rmse(pred_phys, target_phys)
        model_pbias = per_lead_pbias(pred_phys, target_phys)
        model_nse = per_lead_nse(pred_phys, target_phys)
        ...
        for lead in range(args.forecast_steps):
            rows.append(
                EvalSample(
                    ...
                    persistence_pbias=float(persist_pbias[lead]),
                    persistence_nse=float(persist_nse[lead]),
                    model_mae=float(model_mae[lead]),
                    model_rmse=float(model_rmse[lead]),
                    model_pbias=float(model_pbias[lead]),
                    model_nse=float(model_nse[lead]),
                )
            )
    ...
    summary_df["nse_gain_vs_persistence"] = (
        summary_df["model_nse"] - summary_df["persistence_nse"]
    )
    summary_df["abs_pbias_reduction_vs_persistence"] = (
        np.abs(summary_df["persistence_pbias"]) - np.abs(summary_df["model_pbias"])
    )
    summary_path = output_dir / "summary_by_lead.csv"
    summary_df.to_csv(summary_path, index=False)

    config_payload = {
        "soilcast_ckpt": str(args.soilcast_ckpt),
    ...
    overall = {
        ...
        "mean_persistence_pbias": float(per_sample_df["persistence_pbias"].mean()),
        "mean_model_pbias": float(per_sample_df["model_pbias"].mean()),
        "mean_persistence_nse": float(per_sample_df["persistence_nse"].mean()),
        "mean_model_nse": float(per_sample_df["model_nse"].mean()),
    }
    ...
    overall["nse_gain_vs_persistence"] = (
        overall["mean_model_nse"] - overall["mean_persistence_nse"]
    )
    overall["abs_pbias_reduction_vs_persistence"] = (
        abs(overall["mean_persistence_pbias"]) - abs(overall["mean_model_pbias"])
    )
